...
- **paarth skill**: `~/.claude/skills/paarth/SKILL.md` — full skill roster + graphify + mempalace + all best practices
- **paarth-brain agent**: `~/.claude/agents/paarth-brain.md` — PROACTIVELY auto-routes every task to the right skill chain

### Activation

- Say "paarth", "activate all agents", or "full power mode" to explicitly invoke
- The `paarth-brain` agent auto-activates on every build / fix / explore / design / review / ship task
- All skills accessible via paarth: `caveman`, `superpowers:*`, `claude-mem:*`, `ui-ux-pro-max`, `claude-api`, graphify, mempalace
