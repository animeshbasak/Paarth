---
description: Run the cost-aware proxy/model switcher (list / to / back / canary / status / auto on|off). Wraps the `paarth-switch` CLI through the `paarth-switch` skill so canary always runs before a flip.
argument-hint: "[list | to <model> | back | canary <model> | status | auto on|off]"
allowed-tools: ["Bash", "Read"]
---

You were invoked via `/paarth-switch $ARGUMENTS`.

Invoke the **`paarth-switch` skill** with the argument string `$ARGUMENTS`
...
- That state lives in `~/.paarth/`, never `~/.claude/`.
