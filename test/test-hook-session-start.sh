...
HOOK="$SCRIPT_DIR/../hooks/superagent-session-start.py"
...
OUT=$(HOME="$TMPHOME" SUPERAGENT_INJECT_BUDGET_TOKENS=40 python3 "$HOOK" </dev/null)
echo "$OUT" | jq -e '.hookSpecificOutput.additionalContext | contains("## SuperAgent context")' >/dev/null \
...
OUT=$(HOME="$TMPHOME" SUPERAGENT_INJECT_BUDGET_TOKENS=40 SUPERAGENT_INJECT_BUDGET=0 python3 "$HOOK" </dev/null)
echo "$OUT" | jq -e '.hookSpecificOutput.additionalContext | contains("Recent routes (last 5)")' >/dev/null \
  || { echo "FAIL: budget kill switch ignored: $OUT"; exit 1; }

# Dedicated hook kill switch → no output at all
OUT=$(HOME="$TMPHOME" SUPERAGENT_SESSION_CONTEXT=0 python3 "$HOOK" </dev/null || true)
[[ -z "$OUT" ]] || { echo "FAIL: SUPERAGENT_SESSION_CONTEXT=0 still emitted: $OUT"; exit 1; }
