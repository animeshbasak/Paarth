...
[[ "$LINES" == "1" ]] || { echo "FAIL: expected 1 pattern, got $LINES"; cat "$TMPHOME/.paarth/brain/patterns.jsonl"; exit 1; }
...
JSONL
HOME="$TMPHOME" "$PATBIN" promote >/dev/null
REC2=$(grep '"systematic-debugging"' "$TMPHOME/.paarth/brain/patterns.jsonl")
