---
name: paarth
description: Use when starting any session or complex task — master routing skill that activates the right combination of skills. Use when user says "paarth", "activate all agents", "full power mode", or when facing any multi-phase task. Also auto-routes via paarth-brain agent.
---
...
| Tool | Type | Purpose |
|------|------|---------|
| paarth | Claude plugin | This skill — master router |
...
After install: restart Claude Code, then type `paarth` to activate.
