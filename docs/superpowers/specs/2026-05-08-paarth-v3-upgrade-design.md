...
Port ten capabilities from ruflo (`@claude-flow` v3.6) into PAARTH without duplicating the memory and knowledge-graph layers we already get from MemPalace, claude-mem, and graphify. Ship in three dependency-ordered waves so each wave is independently shippable and the classifier becomes self-improving as early as Wave 1.

The lift makes three structural changes:

1. **The classifier learns.** Today's `bin/paarth-classify` is static. Wave 1 wires a pattern store that feeds back successful routes.
2. **Hooks become the integration surface.** Today PAARTH has four hook events wired. Wave 1 raises that to all nine official Claude Code events; Waves 2–3 plug in along the same spine.
3. **Cost becomes enforceable.** Today `paarth-cost` reports totals. Wave 1 adds budget alerts and an auto-downgrade path that consumes the existing `auto-fallback` flag.
...
| 4 | Cost-tracker + budget alerts (4-dim pricing) | 1 | Existing `paarth-cost` reports; Wave 1 enforces. |
...
`auto-fallback, autoplan, bench, cso, fanout, framer-motion, free-llm, investigate, learn, office-hours, plan-ceo-review, plan-design-review, plan-eng-review, review, ship, paarth, paarth-safety, paarth-switch, token-stats, video-craft, webgl-craft`

The `paarth` skill is the master router. `learn` is per-project JSONL — not a learning loop. `paarth-safety` already does the PreToolUse reversibility gate.

### 4.2 Bins (8)

`paarth-chain, paarth-classify, paarth-compile, paarth-cost, paarth-learn, paarth-oneshot, paarth-ship, paarth-switch`. Wave 1 extends `paarth-cost` and `paarth-learn`; later waves add new bins.

### 4.3 Hooks (today)

| Event | Script | Purpose |
|---|---|---|
| PreToolUse | `paarth-safety.py` | Reversibility gate, sensitive-path block |
| PostToolUse:Bash | `paarth-tracker.sh` | Cost JSONL + savings stats |
| SessionStart | `paarth-session-start.py` | Inject 50-line context block |
| Stop | `paarth-distill.sh` | Distill corrections into CLAUDE.md |

Missing (to be added across the waves): `UserPromptSubmit, SubagentStop, Notification, PermissionRequest, PreCompact`.

### 4.4 State root: `~/.paarth/`
...
### 4.5 paarth-brain agent

`agents/paarth-brain.md` — auto-fires on every build/fix/explore/design/review/ship task. Carries its own scoped `hooks:` block so safety enforcement survives subagent dispatch. Wave 2 specialist agents follow the same pattern.
...
| Type | Count | Files |
|---|---|---|
| Bins | 10 | `paarth-aidefence, autopilot, diff, metrics, patterns, sparc, testgen, trace` (8 new) + extensions to `paarth-cost, paarth-learn` |
| Skills | 8 + 2 extended | `aidefence, autopilot, cost-budget, diff-risk, observability, sparc, testgen, paarth-learn-loop` (8 new) + extensions to `paarth, learn` |
...
5 Python hook scripts under `SA:/hooks/`, each ≤100 lines, following `paarth-safety.py`'s shape. Wired into `~/.claude/settings.json` by `install.sh` extending the `hooks.json` template.

| Event | Script | Behaviour |
|---|---|---|
| `UserPromptSubmit` | `paarth-prompt-submit.py` | Classify task; write announce block to stdout `additionalContext`. Wave 2 plugs aidefence into the same script. |
| `SubagentStop` | `paarth-subagent-stop.py` | Append outcome to `~/.paarth/brain/routes.jsonl` with `subagent:true` flag and cost attribution. |
| `Notification` | `paarth-notification.py` | Filter noisy notifications. Pass-through for `error`; drop most `info`. |
| `PermissionRequest` | `paarth-permission.py` | Auto-approve patterns matching `~/.paarth/safety/allow.txt`. Emits `{decision:"allow", reason}`. |
| `PreCompact` | `paarth-precompact.py` | Before window compacts: dump current routes/learnings into claude-mem. |
...
### 6.2 Learning loop — `~/.paarth/brain/patterns.jsonl`
...
**Promote.** Stop hook calls `bin/paarth-patterns promote`. For routes whose `(signal,chain)` hash recurs ≥3 times with `outcome:done`, write or update a pattern record. Increment `useCount`; bump `successRate += 0.03` capped at `1.0`.

**Decay.** Same Stop hook calls `bin/paarth-patterns decay`. Per record:
...
**Protect.** `bin/paarth-patterns protect <id>` flips `protected:true`. Used for hand-curated "don't forget" routes.

**Classifier integration.** `bin/paarth-classify` reads `patterns.jsonl` after `rules.yaml`. The highest-confidence pattern match (signal-overlap × `successRate`) is prepended to the chain — but **only when** `successRate ≥ 0.6 AND useCount ≥ 5`. Below that gate, static rules win. Prevents one-off coincidence routing.
...
`hooks/paarth-tracker.sh` extracts the four token dimensions from the transcript. Anthropic SDK exposes these as `usage.cache_creation_input_tokens` and `usage.cache_read_input_tokens`.

**Pricing config.** `bin/paarth-cost` carries a hardcoded default 4-dimension pricing table (Haiku/Sonnet/Opus × input/output/cache_write/cache_read), overridable via `~/.paarth/cost/pricing.json`. The `pricing_version` field on every record means rate changes don't corrupt historical totals.
...
**Budget config.** New file `~/.paarth/cost/budget.json`:
...
**Alert emit.** Each `paarth-cost` run computes `pct = used_usd / budget.daily_usd`. Crossing a threshold appends to `~/.paarth/cost/alerts.jsonl`:
...
At `0.9` the script drops `~/.paarth/auto-downgrade.flag` containing the target model. The `auto-fallback` skill already reads similar flags for Anthropic↔local switching; it is extended to honor downgrade for in-Anthropic tier shifts (Opus→Sonnet→Haiku).
...
paarth-cost   ─► pct check ─► alerts.jsonl + auto-downgrade.flag
...
**Hook integration.** `paarth-prompt-submit.py` calls `bin/paarth-aidefence scan`, which returns:
...
**Adaptive learning.** `~/.paarth/aidefence/learned.jsonl` stores `{pattern_id, was_accurate, ts}` feedback pairs. EMA update on `effectiveness` (alpha=0.1):
...
**Default off.** `~/.paarth/aidefence/enabled` flag absent at install. User opts in via `paarth-aidefence enable`. Reason: dev workflows mention "ignore previous" in legitimate contexts; too many false positives at first.
...
Each file is ~50–80 lines: frontmatter, system prompt, skill chain hint, per-agent `hooks:` block scoping the safety hook to itself when dispatched as subagent (same pattern as `paarth-brain.md`).

**Routing rules.** `skills/paarth/brain/rules.yaml` extended with five rule blocks mapping keyword → specialist agent. Classifier prefers specialist when chain length > 4 OR complexity = high.

The `paarth-brain` agent remains the master router; specialists are dispatch targets, not replacements.

### 7.3 Observability — JSONL spans + metrics

No OTel libraries. Pure JSONL.

**Span store.** `~/.paarth/obs/spans.jsonl`:
...
**Metric store.** `~/.paarth/obs/metrics.jsonl`:
...
**Trace ID propagation.** `paarth` skill sets `SA_TRACE_ID` env var at chain start. Downstream bins read it; if unset, generate new (root span). Cross-session boundary = new traceId. Don't try to span across SessionStart.

**Bins.**
- `bin/paarth-trace <traceId>` — read spans.jsonl, build parent-child tree, ASCII-print with timing and bottleneck flag (`> p95` of its `op`).
- `bin/paarth-metrics [period]` — aggregate counters/gauges/histograms over period; print table; p50/p95/p99 = sorted-position lookup.
...
**Hook integration.** `PreToolUse` adds span-start, `PostToolUse` adds span-end. Folded into the existing `paarth-tracker.sh` rather than new scripts.
...
2. `~/.paarth/brain/routes.jsonl` records with `outcome:halt` (resumeable).
3. User-supplied `tasks.md` in `cwd`.

**Predict.** `bin/paarth-autopilot predict` reads pending tasks plus `~/.paarth/brain/patterns.jsonl` (Wave 1). Returns `{action, confidence, target}` where confidence = matched `pattern.successRate`.

**Confidence threshold.** `> 0.7` → execute prediction. `≤ 0.7` → fall back to highest-priority pending task. Empty → disable autopilot.

**Cache-warm wakeup.** Calls `ScheduleWakeup({delaySeconds: 270, reason: "autopilot iter"})`. Stays under Anthropic's 300s prompt-cache TTL. Tunable via `PAARTH_CACHE_TTL_S` (default 270).
...
**Coverage adapter** (`bin/paarth-testgen`):
...
| Override | `~/.paarth/testgen/cov-cmd.txt` | user-supplied path |

**Gap detection.** `gap = targetCoverage - currentCoverage` per file. Sort by `gap × LOC` (impact). Top-N report.

**Per-project threshold.** `~/.paarth/testgen/min-coverage.txt` (default 70). Project-level override prevents a hard global gate from blocking legacy code.

**Test scaffolding.** For each gapped file, `paarth-testgen suggest <file>` outputs a markdown skeleton:
...
**State.** `~/.paarth/testgen/last-report.json` cached for `ship`/`review` to consult without rerun.
...
**State.** `~/.paarth/diff/last.json` cached for `ship`/`review` chain.
...
`bin/paarth-classify` reads in order:

1. `skills/paarth/brain/rules.yaml` (existing static keyword rules)
2. `~/.paarth/brain/patterns.jsonl` (Wave 1 learned routes)
...
| `brain/patterns.jsonl` | never (dedup-compacted) | manual `paarth-patterns prune` |
| `aidefence/learned.jsonl` | monthly | 365 days |
| `cost/alerts.jsonl` | weekly | 90 days |
| `sparc/<slug>/` | manual (per-feature) | user-driven |

Rotation runs on `Stop` hook (cheap, runs at session end). Prune runs once daily (first hook of the day). Lock via existing `~/.paarth/*.lock` pattern.

### 9.3 Configurability — `defaults.toml`

Single source of truth: `~/.paarth/defaults.toml`. All magic numbers env-tunable, file-overridable, code-defaulted.
...
Env override pattern: `PAARTH_LEARNING_DECAY_RATE_PER_HOUR=0.01`. Bins parse `defaults.toml` once, then check env.
...
| From | To | Migrations |
|---|---|---|
| v2.3 → v2.4 (Wave 1) | (a) Backup `~/.paarth/cost/calls.jsonl` to `calls.v1.jsonl.bak`. (b) New writes use v2 schema; v1 records auto-detected by absence of `input_tokens`. **v1 cost computation:** treat `tokens` as `output_tokens` (conservative — Anthropic SDK historically reported this field as completion tokens), set the other three dimensions to 0. Records keep their original totals; future analytics splits new vs old via `pricing_version` presence. (c) Create `~/.paarth/cost/budget.json` with conservative defaults. (d) Append 5 hook entries to `~/.claude/settings.json`; idempotent. (e) Create empty `~/.paarth/brain/patterns.jsonl`. **Pre-existing `routes.jsonl` records ARE read by the promote loop on first run** — pattern store backfills naturally from history (subject to the `useCount ≥ 5` gate, so noise is filtered). |
| v2.4 → v2.5 (Wave 2) | (a) Drop 5 agent files into `~/.claude/agents/`. (b) Create `~/.paarth/{aidefence,autopilot,obs}/` skeletons. (c) AIDefence stays disabled. Print one-liner: `paarth-aidefence enable` to opt in. |
| v2.5 → v2.6 (Wave 3) | (a) Create `~/.paarth/{sparc,testgen,diff}/`. (b) New skills + bins. No state migration. |

**Idempotency contract.** Running install on already-installed wave is a no-op. Verified via marker files: `~/.paarth/.wave-{1,2,3}.installed`.

**Rollback.** Manual: rename `~/.paarth/.wave-N.installed`, revert hook entries. We do not delete user state on rollback.

**Backups.** Each wave install creates `~/.paarth/.backups/<ts>-pre-wave-N/` snapshot of mutated files. Garbage-collected after 30 days.

### 9.5 Adapter recompile

`bin/paarth-compile` (621 lines today) emits adapters for: aider, codex, continue, copilot, cursor, gemini, windsurf.
...
- Research doc: `/tmp/paarth-ruflo-research.md` (1231 lines, 10 picks fully extracted).
- ruflo source: `/Users/animeshbasak/Desktop/ai-lab/projects/references/ruflo/`.
- v2 plan: `docs/superpowers/plans/2026-04-24-paarth-v2.md`.
