# PAARTH v2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship PAARTH v2 — a single `/paarth <task>` entrypoint with an AI routing brain, a role-based skill roster imported from gstack, a verification bench harness, auto-learning CLAUDE.md, cost intelligence, skill DAGs, and a privacy mode. Preserves v1's infra edge (mempalace, graphify, token tracker).

**Architecture:**
- **Single entrypoint** — `/paarth <task>` loads a router skill whose body is a decision tree. Claude reads the task, classifies, announces a skill chain, executes it. No subagent routing — stays in-context.
...
- **Self-improvement loop** — Stop hook distills corrections into CLAUDE.md; routing decisions log to `~/.paarth/brain/routes.jsonl` and bias future selection; bench harness scores the brain on 20 golden prompts.
...
- **Skill DAGs** — YAML-declared chains under `skills/paarth/chains/` with resume-on-fail; parallel fanout primitive.
- **Privacy mode** — `--local-only` flag gates outbound calls; all state under `~/.paarth/` (single root, no scatter).
...
### Task 0.1: Create `~/.paarth/` state root + migration shim

**Files:**
- Create: `hooks/paarth-state-init.sh`
...
```bash
chmod +x hooks/paarth-state-init.sh
bash hooks/paarth-state-init.sh
test -d ~/.paarth/brain && echo OK
```
Expected: `OK` printed; `~/.paarth/{brain,bench,learnings,chains,cost,logs}` exist.
...
bash "$REPO_DIR/hooks/paarth-state-init.sh"
```

- [ ] **Step 4: Commit**

```bash
git add hooks/paarth-state-init.sh install.sh
git commit -m "feat(v2): add ~/.paarth state root + install step"
```
...
**Files:**
- Create: `skills/paarth/brain/rules.yaml`
...
```bash
python3 -c "import yaml; yaml.safe_load(open('skills/paarth/brain/rules.yaml'))" && echo OK
```
Expected: `OK`.
...
```bash
git add skills/paarth/brain/rules.yaml
...
Create `bin/paarth-classify` (requires `yq` — check with `command -v yq` and error clearly if missing):
```bash
#!/usr/bin/env bash
# Usage: paarth-classify "task string" -> JSON array of skill chain
set -euo pipefail
TASK="${1:?task required}"
RULES="${PAARTH_RULES:-$HOME/.claude/skills/paarth/brain/rules.yaml}"
[[ -f "$RULES" ]] || RULES="$(dirname "$0")/../skills/paarth/brain/rules.yaml"
...
while IFS= read -r line; do
  ...
  ACTUAL=$(./bin/paarth-classify "$PROMPT")
...
bash bench/run.sh
```
Expected: `PASS=20 FAIL=0`. If any miss, tune rules in [skills/paarth/brain/rules.yaml](skills/paarth/brain/rules.yaml), re-run until green.

- [ ] **Step 6: Commit**

```bash
git add bin/paarth-classify test/test-classify.sh bench/run.sh
git commit -m "feat(v2): add classifier + bench runner, 20/20 golden prompts pass"
```

---

### Task 1.4: Rewrite `/paarth` skill as router

**Files:**
- Modify: `skills/paarth/SKILL.md` (full rewrite)
- Backup: `skills/paarth/SKILL.v1.md` (preserve old content)

- [ ] **Step 1: Back up v1**

```bash
cp skills/paarth/SKILL.md skills/paarth/SKILL.v1.md
```

- [ ] **Step 2: Write the new router skill**

Overwrite `skills/paarth/SKILL.md` with:
````markdown
---
name: paarth
...
**6. Log.** After completion (or halt), append to `~/.paarth/brain/routes.jsonl`:
```json
...
python3 -c "
import re, yaml, sys
t = open('skills/paarth/SKILL.md').read()
m = re.match(r'^---\n(.*?)\n---', t, re.S)
assert m, 'no frontmatter'
fm = yaml.safe_load(m.group(1))
assert fm['name'] == 'paarth'
...
```bash
git add skills/paarth/SKILL.md skills/paarth/SKILL.v1.md
git commit -m "feat(v2): rewrite /paarth as router with inline classifier"
```

---

### Task 1.5: Wire `paarth-classify` binary into `install.sh`
...
```bash
# Step X: Install paarth CLI
mkdir -p "$HOME/.local/bin"
cp "$REPO_DIR/bin/paarth-classify" "$HOME/.local/bin/paarth-classify"
chmod +x "$HOME/.local/bin/paarth-classify"
echo "installed: ~/.local/bin/paarth-classify"
```
...
paarth-classify "fix the bug" | jq .
```
...
```bash
git add install.sh
git commit -m "feat(v2): install paarth-classify to ~/.local/bin"
```
...
**Files:**
- Modify: `bin/paarth-classify` (add history read)
...
cat > ~/.paarth/brain/routes.jsonl <<'EOF'
...
HINT=$(./bin/paarth-classify "fix the flake" 2>&1 | jq -r '.hint // ""')
...
After computing `FINAL` in `bin/paarth-classify`, add before `echo`:
```bash
HISTORY="$HOME/.paarth/brain/routes.jsonl"
HINT="null"
if [[ -f "$HISTORY" ]]; then
...
```bash
git add bin/paarth-classify test/test-classify.sh
...
Each imported skill is a new file under `skills/<name>/SKILL.md`, rewritten for paarth context (no gstack branding, references paarth infra).
...
Source: `/tmp/gstack-analysis/plan-eng-review/SKILL.md`. Keep the architecture-lock-in questions (data flow, edge cases, test coverage, failure modes). Remove gstack chrome. Reference paarth's `claude-mem:smart-explore` and `graphify query` for code discovery.
...
2. Run `bin/paarth-ship` for rebase.
...
```bash
chmod +x bin/paarth-ship
git add skills/ship/SKILL.md bin/paarth-ship
...
Stores per-project persistent learnings at `~/.paarth/learnings/<project-hash>.jsonl`.

`bin/paarth-learn`:
```bash
#!/usr/bin/env bash
# Usage: paarth-learn add "<text>" | list | search "<q>"
set -euo pipefail
ROOT="$HOME/.paarth/learnings"
mkdir -p "$ROOT"
...
case "${1:-}" in
  ...
  search)
    ...
    ;;
  *) echo "usage: paarth-learn {add|list|search}" ; exit 1 ;;
esac
```
...
```bash
chmod +x bin/paarth-learn
git add skills/office-hours/SKILL.md skills/cso/SKILL.md skills/learn/SKILL.md bin/paarth-learn
...
Find the section linking `skills/paarth` and `skills/token-stats`. Replace with a loop:
```bash
for skill in paarth token-stats webgl-craft plan-ceo-review plan-eng-review plan-design-review autoplan review investigate ship office-hours cso learn; do
...
```bash
ACTUAL=$(./bin/paarth-classify "$PROMPT" | jq -c .chain)
...
3. If < 20/20, propose rule edits in `skills/paarth/brain/rules.yaml`.
...
**Files:**
- Create: `hooks/paarth-distill.sh`
...
{
  "Stop": [
    { "matcher": "*", "hooks": [{ "type": "command", "command": "bash $CLAUDE_PLUGIN_ROOT/hooks/paarth-distill.sh || true" }] }
  ]
}
```
...
```bash
chmod +x hooks/paarth-distill.sh
echo '{"transcript_path":"/tmp/fake.jsonl"}' | bash hooks/paarth-distill.sh
```
...
```bash
git add hooks/paarth-distill.sh .claude/settings.json
...
**Files:**
- Modify: `hooks/paarth-statusline.sh`

- [ ] **Step 1: Read current statusline**

```bash
cat hooks/paarth-statusline.sh
```
...
```bash
echo '{"transcript_tokens":340000}' | bash hooks/paarth-statusline.sh | grep -q "⚠️" && echo OK
```
Expected: `OK`.
...
**Files:**
- Modify: `hooks/paarth-distill.sh` (dual-write to `~/.paarth/learnings/`)
...
```bash
git add hooks/paarth-distill.sh
git commit -m "feat(v2): distill hook also writes to ~/.paarth/learnings/"
```
...
**Files:**
- Create: `skills/paarth/chains/ship-v2.yaml`
- Create: `skills/paarth/chains/feature-build.yaml`
- Create: `bin/paarth-chain`

- [ ] **Step 1: Write example chain**

`skills/paarth/chains/ship-v2.yaml`:
```yaml
name: ship-v2
...
`skills/paarth/chains/feature-build.yaml`:
```yaml
name: feature-build
...
```bash
#!/usr/bin/env bash
# Usage: paarth-chain <chain-name>
# Emits a JSON array of skill names to stdout; the /paarth skill executes them.
set -euo pipefail
NAME="${1:?chain required}"
DIR="${PAARTH_CHAINS:-$HOME/.claude/skills/paarth/chains}"
FILE="$DIR/$NAME.yaml"
...
```bash
chmod +x bin/paarth-chain
PAARTH_CHAINS="$PWD/skills/paarth/chains" ./bin/paarth-chain ship-v2
```
...
```bash
git add skills/paarth/chains/ bin/paarth-chain
...
**Files:**
- Create: `bin/paarth-cost`
- Modify: `hooks/paarth-tracker.sh` (emit model + tokens per call)

- [ ] **Step 1: Extend tracker to log per-call records**

In `hooks/paarth-tracker.sh`, after the existing jq atomic-write block, append a line to `~/.paarth/cost/calls.jsonl`:
```bash
# Per-call cost log
COST_FILE="$HOME/.paarth/cost/calls.jsonl"
...
```bash
#!/usr/bin/env bash
# Usage: paarth-cost [today|week|all]
set -euo pipefail
RANGE="${1:-today}"
...
```bash
chmod +x bin/paarth-cost
echo '{"ts":"2026-04-24T10:00:00+00:00","project":"'$PWD'","tool":"test","tokens":120000,"model":"claude-opus-4-7"}' >> ~/.paarth/cost/calls.jsonl
./bin/paarth-cost today
```
...
```bash
git add bin/paarth-cost hooks/paarth-tracker.sh
...
When `LOCAL_ONLY=1`, skip cloud-optional installs (graphify LLM-backed features, anything that phones home), and write a marker file `~/.paarth/local-only` that hooks can check to gate outbound calls.

- [ ] **Step 2: Honor marker in tracker hook**

In `hooks/paarth-tracker.sh`, short-circuit any remote calls if `~/.paarth/local-only` exists.

- [ ] **Step 3: Commit**

```bash
git add install.sh hooks/paarth-tracker.sh
git commit -m "feat(v2): --local-only install mode + marker file honored by hooks"
```

---

### Task 6.2: Consolidate all state under `~/.paarth/`

**Files:**
- Modify: every hook/bin that references `~/.claude/paarth-*` or `~/.gstack/`

- [ ] **Step 1: Grep current paths**

```bash
grep -r --include='*.sh' '~/.claude/paarth\|HOME/.claude/paarth' hooks/ bin/ install.sh | tee /tmp/legacy-paths.txt
```

- [ ] **Step 2: Migrate each reference**

For each match, change to `~/.paarth/<subdir>/...`. Example:
- `~/.claude/paarth-stats.json` → `~/.paarth/stats.json`
- `~/.claude/paarth-tracker.log` → `~/.paarth/logs/tracker.log`

Update the migration shim in `hooks/paarth-state-init.sh` (Task 0.1) to move any remaining legacy files on next install.
...
```bash
cd /Users/animeshbasak/Desktop/ai-lab/projects/paarth
git add -A
git commit -m "refactor(v2): consolidate all state under ~/.paarth/"
```
...
### Added
- `/paarth <task>` single-entrypoint AI router with inline classifier + learning-loop bias
...
- Cost intelligence: $ conversion, model-mix coach via `paarth-cost`
...
- Consolidated state under `~/.paarth/`
- ETHOS.md — 5 guiding principles, auto-referenced by every skill

### Changed
- `/paarth` skill was a stack activator; now a task router
- `~/.claude/paarth-*.json` paths migrated to `~/.paarth/`
```

- [ ] **Step 2: Update README**

Add a v2 section at the top showcasing `/paarth <task>`. Minimum: 1 "hook" sentence, 1 demo block, 1 table listing skills, 1 install command, 1 bench result.
...
```bash
git tag -a v2.0.0 -m "PAARTH v2 — AI brain + role skills + bench + learning loop"
```
...
```bash
gh release create v2.0.0 --title "PAARTH v2.0.0" --notes-file CHANGELOG.md
```
...
| AI brain — `/paarth <task>` auto-routing | Tasks 1.1 – 1.6 |
...
| State consolidation `~/.paarth/` | Tasks 0.1, 6.2 |
...
**Type consistency:**
- `paarth-classify` emits `{chain, hint}` consistently after Task 1.6 (tests updated in Step 3 of that task).
- Chain names in `rules.yaml`, `bench/prompts.jsonl`, and `skills/paarth/chains/*.yaml` all use kebab-case without the `superpowers:` prefix (the router strips/prepends as needed).
- State paths use `~/.paarth/<subdir>/` uniformly after Phase 6.

---

## Execution Handoff

Plan complete and saved to `docs/superpowers/plans/2026-04-24-paarth-v2.md`. Two execution options:
