# PAARTH v3 — Wave 1 (Foundation, v2.4.0) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Land the three Wave 1 components from the v3 spec (hooks lifecycle, learning loop, cost-tracker schema bump) so the PAARTH classifier becomes self-improving and budget enforcement works.

**Architecture:** Three concurrent threads of work (5 net-new hooks; learning-loop store + classifier wiring; cost-tracker 4-dim schema + budget alerts) all write into `~/.paarth/` subdirs. Each thread is independently testable. Spec source: `docs/superpowers/specs/2026-05-08-paarth-v3-upgrade-design.md` §6 + §9.
...
### New files
- `bin/paarth-patterns` — pattern store CLI (list, promote, decay, protect, prune subcommands)
- `hooks/paarth-prompt-submit.py` — UserPromptSubmit hook (classify + announce)
- `hooks/paarth-subagent-stop.py` — SubagentStop hook (route log + cost attribution)
- `hooks/paarth-notification.py` — Notification hook (filter)
- `hooks/paarth-permission.py` — PermissionRequest hook (auto-allow from safety/allow.txt)
- `hooks/paarth-precompact.py` — PreCompact hook (export to claude-mem)
- `skills/paarth-learn-loop/SKILL.md` — learning loop user-facing skill
...
### Modified files
- `bin/paarth-cost` — 4-dim pricing, budget alerts, auto-downgrade.flag drop, v1 auto-detect
- `bin/paarth-classify` — read patterns.jsonl, gate at successRate≥0.6 + useCount≥5
- `hooks/paarth-tracker.sh` — write v2 records (4-dim tokens) to calls.jsonl
- `hooks/paarth-distill.sh` — call patterns promote+decay at Stop
- `hooks/paarth-state-init.sh` — scaffold new dirs + defaults.toml + budget.json
...
- `skills/auto-fallback/SKILL.md` — honor `~/.paarth/auto-downgrade.flag`
...
- `~/.paarth/defaults.toml` — magic-number config
- `~/.paarth/cost/budget.json` — budget config (conservative defaults)
- `~/.paarth/cost/pricing.json` — optional override (created empty)
- `~/.paarth/brain/patterns.jsonl` — empty file (touched at install)
- `~/.paarth/brain/protected-patterns.jsonl` — empty file
- `~/.paarth/cost/alerts.jsonl` — touched
- `~/.paarth/.wave-1.installed` — idempotency marker
...
**Files:**
- Modify: `hooks/paarth-state-init.sh` (add new dirs, defaults.toml seeding, budget.json default)
...
Replace the body of `hooks/paarth-state-init.sh` with:

```bash
#!/usr/bin/env bash
# Idempotent: creates ~/.paarth/ and subdirs, migrates legacy paths.
set -euo pipefail
ROOT="${HOME}/.paarth"
...
LEGACY="${HOME}/.claude/paarth-stats.json"
NEW="$ROOT/stats.json"
...
# Default pricing.json — empty placeholder; bin/paarth-cost falls back to hardcoded table
...
# PAARTH defaults — env vars override (e.g. PAARTH_LEARNING_DECAY_RATE_PER_HOUR)
[learning]
...
# paarth-safety pre-approved patterns. One regex per line.
# Examples (commented out — uncomment if you want them allowed silently):
# ^git push --force-with-lease\b
# ^rm -rf /tmp/paarth-bench-
ALLOWEOF
fi

echo "paarth state root ready: $ROOT"
```
...
```bash
git add hooks/paarth-state-init.sh test/test-state-init.sh
...
**Files:**
- Modify: `bin/paarth-cost` (extend pricing table, support v2 records, fallback v1 detection)
...
- [ ] **Step 3: Implement v2 pricing in paarth-cost**

Replace the python heredoc inside `bin/paarth-cost`. The full new file:

```bash
#!/usr/bin/env bash
# Usage: paarth-cost [today|week|all] [--json]
# Reads ~/.paarth/cost/calls.jsonl (mixed v1/v2 records), groups by model, prints $ cost table + coach note.
# --json emits machine-readable output (used by paarth-limit-watch.sh hook).
set -euo pipefail
RANGE="today"
JSON_MODE=0
for arg in "$@"; do
...
FILE="$HOME/.paarth/cost/calls.jsonl"
...
# 4-dim pricing per 1M tokens (Anthropic 2026-Q2). Override at ~/.paarth/cost/pricing.json
DEFAULT_PRICING = {
...
override_path = os.path.join(home, ".paarth", "cost", "pricing.json")
...
plan_limit = float(os.environ.get('PAARTH_PLAN_LIMIT_USD', '20'))
...
```bash
git add bin/paarth-cost test/test-cost-v2.sh
...
**Files:**
- Modify: `hooks/paarth-tracker.sh` (extract 4-dim usage from PostToolUse stdin, write v2 record)
...
In `hooks/paarth-tracker.sh`, find both cost-log emission blocks (one in the non-Bash branch, one at the bottom of the Bash branch — search for `"# Per-call cost log"` and `"Non-Bash tools: estimate tokens"`).
...
emit_cost_record() {
  ...
  local cost_file="$HOME/.paarth/cost/calls.jsonl"
...
```bash
git add hooks/paarth-tracker.sh test/test-tracker-v2.sh
...
**Files:**
- Create: `bin/paarth-cost-alerts` (separate small bin to avoid bloating paarth-cost; called from hooks/cron)
...
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ALERTS_BIN="$SCRIPT_DIR/../bin/paarth-cost-alerts"
COST_BIN="$SCRIPT_DIR/../bin/paarth-cost"
...
LATEST=$(tail -n1 "$TMPHOME/.paarth/cost/alerts.jsonl")
...
[[ -f "$TMPHOME/.paarth/auto-downgrade.flag" ]] || { echo "FAIL: auto-downgrade.flag missing"; rm -rf "$TMPHOME"; exit 1; }
grep -q "sonnet" "$TMPHOME/.paarth/auto-downgrade.flag" || { echo "FAIL: flag missing target"; rm -rf "$TMPHOME"; exit 1; }

# Idempotency: running again should not duplicate the same threshold-tier alert
HOME="$TMPHOME" PATH="$SCRIPT_DIR/../bin:$PATH" "$ALERTS_BIN" >/dev/null
COUNT=$(grep -c '"level":"critical"' "$TMPHOME/.paarth/cost/alerts.jsonl" || echo 0)
...
Expected: `bash: bin/paarth-cost-alerts: No such file` (or exec failure).

- [ ] **Step 3: Implement cost-alerts bin**

Create `bin/paarth-cost-alerts`:

```bash
#!/usr/bin/env bash
# paarth-cost-alerts — emit threshold-crossing alerts and drop auto-downgrade.flag.
...
COST_BIN="$(dirname "$0")/paarth-cost"

[[ -f "$BUDGET" ]] || { echo "no budget config (expected $BUDGET)" >&2; exit 0; }
[[ -x "$COST_BIN" ]] || COST_BIN="paarth-cost"
...
```bash
chmod +x bin/paarth-cost-alerts
```
...
```bash
git add bin/paarth-cost-alerts test/test-cost-alerts.sh
...
Expected: `bin/paarth-patterns: No such file`.
...
```bash
#!/usr/bin/env bash
# paarth-patterns — pattern store CLI for the PAARTH learning loop.
# Subcommands: list, promote, decay, protect, prune.
set -euo pipefail

ROOT="$HOME/.paarth/brain"
...
usage() {
  cat <<EOF
Usage: paarth-patterns <subcommand> [flags]
...
```bash
git add bin/paarth-patterns test/test-patterns.sh
git commit -m "feat(patterns): scaffold paarth-patterns bin (list + help)"
```
...
**Files:**
- Modify: `bin/paarth-patterns` (implement `promote`)
...
[[ "$LINES" == "1" ]] || { echo "FAIL: expected 1 pattern, got $LINES"; cat "$TMPHOME/.paarth/brain/patterns.jsonl"; rm -rf "$TMPHOME"; exit 1; }

REC=$(cat "$TMPHOME/.paarth/brain/patterns.jsonl")
...
HOME="$TMPHOME" "$PATBIN" promote >/dev/null
LINES2=$(wc -l < "$TMPHOME/.paarth/brain/patterns.jsonl" | tr -d ' ')
...
In `bin/paarth-patterns`, replace the `promote|decay|protect|prune)` placeholder branch. Add this dedicated block, in this order (decay/protect/prune still placeholders):
...
PROMOTE_MIN_USES = int(os.environ.get('PAARTH_LEARNING_PROMOTE_MIN_USES', '3'))
...
```bash
git add bin/paarth-patterns test/test-patterns-promote.sh
...
**Files:**
- Modify: `bin/paarth-patterns` (implement `decay`)
...
PA=$(grep '"p-aaaa"' "$TMPHOME/.paarth/brain/patterns.jsonl" | jq -r '.successRate')
...
grep -q '"p-bbbb"' "$TMPHOME/.paarth/brain/patterns.jsonl" \
  && { echo "FAIL: p-bbbb should have been pruned"; rm -rf "$TMPHOME"; exit 1; } || true

# p-cccc: floored at 0.3 (protected)
PC=$(grep '"p-cccc"' "$TMPHOME/.paarth/brain/patterns.jsonl" | jq -r '.successRate')
...
In `bin/paarth-patterns`, replace the `decay|protect|prune)` placeholder. Add the `decay)` branch above the `protect|prune)` placeholder:
...
path = sys.argv[1]
DECAY_PER_HOUR = float(os.environ.get('PAARTH_LEARNING_DECAY_RATE_PER_HOUR', '0.005'))
MIN_CONF = float(os.environ.get('PAARTH_LEARNING_MIN_CONFIDENCE', '0.1'))
PROTECTED_FLOOR = float(os.environ.get('PAARTH_LEARNING_PROTECTED_FLOOR', '0.3'))
...
```bash
git add bin/paarth-patterns test/test-patterns-decay.sh
...
**Files:**
- Modify: `bin/paarth-patterns` (implement `protect`, `prune`)
...
PROT=$(grep '"p-1111"' "$TMPHOME/.paarth/brain/patterns.jsonl" | jq -r '.protected')
...
LINES=$(wc -l < "$TMPHOME/.paarth/brain/patterns.jsonl" | tr -d ' ')
[[ "$LINES" == "1" ]] || { echo "FAIL: prune kept $LINES lines, want 1"; cat "$TMPHOME/.paarth/brain/patterns.jsonl"; rm -rf "$TMPHOME"; exit 1; }
grep -q '"p-1111"' "$TMPHOME/.paarth/brain/patterns.jsonl" || { echo "FAIL: protected p-1111 was pruned"; rm -rf "$TMPHOME"; exit 1; }
...
In `bin/paarth-patterns`, replace the `protect|prune)` placeholder branch with:

```bash
  protect)
    pid="${2:-}"
    [[ -z "$pid" ]] && { echo "Usage: paarth-patterns protect <pattern-id>" >&2; exit 1; }
...
```bash
git add bin/paarth-patterns test/test-patterns-protect-prune.sh
...
**Files:**
- Modify: `bin/paarth-classify` (read patterns.jsonl, prepend matched chain when gate met)
...
- [ ] **Step 3: Modify `bin/paarth-classify`**

Locate the python heredoc inside `bin/paarth-classify`. Make these three changes inside the heredoc (the bash `python3 - "$TASK" "$RULES_FILE" "$HISTORY_HINT" <<'PYEOF'` block):
...
# Read ~/.paarth/brain/patterns.jsonl. If a pattern's signal-token overlap
# with the task is ≥1 AND successRate ≥ 0.6 AND useCount ≥ 5, prepend its chain
# (after always_first, before rule-matched skills).
PATTERNS_PATH = os.path.expanduser("~/.paarth/brain/patterns.jsonl")
GATE_SR = float(os.environ.get('PAARTH_LEARNING_CLASSIFIER_GATE_SUCCESS_RATE', '0.6'))
GATE_USE = int(os.environ.get('PAARTH_LEARNING_CLASSIFIER_GATE_USE_COUNT', '5'))
...
```bash
git add bin/paarth-classify test/test-classify-patterns.sh
...
**Files:**
- Modify: `hooks/paarth-distill.sh` (append patterns promote+decay invocation)
...
- [ ] **Step 3: Modify `hooks/paarth-distill.sh`**

Append this block to the END of `hooks/paarth-distill.sh` (after whatever existing distillation logic is in place):

```bash

# ── Wave 1: pattern store maintenance ─────────────────────────────────────────
# Resolve paarth-patterns: prefer PATH, then script-relative bin/, else skip.
PATBIN=""
...
```bash
git add hooks/paarth-distill.sh test/test-distill-patterns.sh
git commit -m "feat(hooks): Stop hook calls patterns promote+decay"
```

---

## Task 12: New hook — UserPromptSubmit (`paarth-prompt-submit.py`)

**Files:**
- Create: `hooks/paarth-prompt-submit.py`
...
# Must contain additionalContext mentioning PAARTH
...
Expected: `python3: can't open file 'hooks/paarth-prompt-submit.py'`.
...
def main():
    ...
    classifier = shutil.which("paarth-classify") or os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "bin", "paarth-classify"
    )
...
```bash
chmod +x hooks/paarth-prompt-submit.py
```
...
```bash
git add hooks/paarth-prompt-submit.py test/test-hook-prompt-submit.sh
git commit -m "feat(hooks): UserPromptSubmit hook — classify + announce route"
```

---

## Task 13: New hook — SubagentStop (`paarth-subagent-stop.py`)

**Files:**
- Create: `hooks/paarth-subagent-stop.py`
...
Expected: `python3: can't open file 'hooks/paarth-subagent-stop.py'`.
...
```python
#!/usr/bin/env python3
"""SubagentStop hook — log subagent outcome to ~/.paarth/brain/routes.jsonl
...
def main():
    ...
    record = {
        ...
        "outcome": outcome,
        "user_override": "no",
        "backend": os.environ.get("PAARTH_BACKEND", "anthropic"),
        "subagent": True,
        "session_id": session_id,
    }

    routes = os.path.expanduser("~/.paarth/brain/routes.jsonl")
...
```bash
chmod +x hooks/paarth-subagent-stop.py
```
...
```bash
git add hooks/paarth-subagent-stop.py test/test-hook-subagent-stop.sh
git commit -m "feat(hooks): SubagentStop — log subagent outcomes to routes.jsonl"
```

---

## Task 14: New hook — Notification (`paarth-notification.py`)

**Files:**
- Create: `hooks/paarth-notification.py`
...
Expected: `python3: can't open file 'hooks/paarth-notification.py'`.
...
```bash
chmod +x hooks/paarth-notification.py
```
...
```bash
git add hooks/paarth-notification.py test/test-hook-notification.sh
git commit -m "feat(hooks): Notification filter — drop noisy info, pass error/warning through"
```

---

## Task 15: New hook — PermissionRequest (`paarth-permission.py`)

**Files:**
- Create: `hooks/paarth-permission.py`
...
Expected: `python3: can't open file 'hooks/paarth-permission.py'`.
...
```python
#!/usr/bin/env python3
"""PermissionRequest hook — auto-allow Bash commands matching ~/.paarth/safety/allow.txt.
...
ALLOW_PATH = os.path.expanduser("~/.paarth/safety/allow.txt")
...
```bash
chmod +x hooks/paarth-permission.py
```
...
```bash
git add hooks/paarth-permission.py test/test-hook-permission.sh
git commit -m "feat(hooks): PermissionRequest — auto-allow safety/allow.txt regexes"
```

---

## Task 16: New hook — PreCompact (`paarth-precompact.py`)

**Files:**
- Create: `hooks/paarth-precompact.py`
...
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HOOK="$SCRIPT_DIR/../hooks/paarth-precompact.py"
...
ls "$TMPHOME"/.paarth/logs/precompact-*.jsonl >/dev/null 2>&1 \
...
Expected: `python3: can't open file 'hooks/paarth-precompact.py'`.
...
Writes a small log file under ~/.paarth/logs/precompact-<ts>.jsonl containing
...
SNAPSHOT_DIR = os.path.expanduser("~/.paarth/logs")
ROUTES = os.path.expanduser("~/.paarth/brain/routes.jsonl")
...
```bash
chmod +x hooks/paarth-precompact.py
```
...
```bash
git add hooks/paarth-precompact.py test/test-hook-precompact.sh
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {"type": "command", "command": "python3 \"$HOME/.claude/paarth-safety.py\""}
        ]
      }
    ],
    "SessionStart": [
      {
        "matcher": "*",
        "hooks": [
          {"type": "command", "command": "python3 \"$HOME/.claude/paarth-session-start.py\""}
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {"type": "command", "command": "bash \"$HOME/.claude/paarth-tracker.sh\""}
        ]
      }
    ],
    "Stop": [
      {
        "matcher": "*",
        "hooks": [
          {"type": "command", "command": "bash \"$HOME/.claude/paarth-distill.sh\" || true"}
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        "matcher": "*",
        "hooks": [
          {"type": "command", "command": "python3 \"$HOME/.claude/paarth-prompt-submit.py\""}
        ]
      }
    ],
    "SubagentStop": [
      {
        "matcher": "*",
        "hooks": [
          {"type": "command", "command": "python3 \"$HOME/.claude/paarth-subagent-stop.py\""}
        ]
      }
    ],
    "Notification": [
      {
        "matcher": "*",
        "hooks": [
          {"type": "command", "command": "python3 \"$HOME/.claude/paarth-notification.py\""}
        ]
      }
    ],
    "PermissionRequest": [
      {
        "matcher": "Bash",
        "hooks": [
          {"type": "command", "command": "python3 \"$HOME/.claude/paarth-permission.py\""}
        ]
      }
    ],
    "PreCompact": [
      {
        "matcher": "*",
        "hooks": [
          {"type": "command", "command": "python3 \"$HOME/.claude/paarth-precompact.py\""}
        ]
      }
    ]
  }
...
info "Installing 9 PAARTH hooks (4 existing + 5 net-new)..."

# Source files in repo
declare -A HOOK_SRCS=(
  [paarth-safety.py]="$SCRIPT_DIR/hooks/paarth-safety.py"
  [paarth-session-start.py]="$SCRIPT_DIR/hooks/paarth-session-start.py"
  [paarth-tracker.sh]="$SCRIPT_DIR/hooks/paarth-tracker.sh"
  [paarth-distill.sh]="$SCRIPT_DIR/hooks/paarth-distill.sh"
  [paarth-prompt-submit.py]="$SCRIPT_DIR/hooks/paarth-prompt-submit.py"
  [paarth-subagent-stop.py]="$SCRIPT_DIR/hooks/paarth-subagent-stop.py"
  [paarth-notification.py]="$SCRIPT_DIR/hooks/paarth-notification.py"
  [paarth-permission.py]="$SCRIPT_DIR/hooks/paarth-permission.py"
  [paarth-precompact.py]="$SCRIPT_DIR/hooks/paarth-precompact.py"
)
...
const wirings = [
  ...
  { event: 'PostToolUse',      matcher: 'Bash|Edit|Write|MultiEdit|Read|Grep|Glob',cmd: 'bash "$HOME/.claude/paarth-tracker.sh"' },
  { event: 'Stop',             matcher: '*',                                       cmd: 'bash "$HOME/.claude/paarth-distill.sh" || true' },
...
console.log('All 9 PAARTH hooks wired (idempotent)');
NODE
...
{"id": 27, "prompt": "promote this routing pattern so we don't have to rediscover it next time", "archetype": "learning", "expected_chain": ["mempalace-wake", "paarth-learn-loop", "learn"]}
{"id": 28, "prompt": "auto-approve git push --force-with-lease in this repo", "archetype": "permission", "expected_chain": ["mempalace-wake", "paarth-safety"]}
...
Add (or extend) entries in `skills/paarth/brain/rules.yaml` so the new keywords route correctly. Append the following rules near the end of the `rules:` list (preserve the existing ones):
...
```bash
git add bench/prompts.jsonl skills/paarth/brain/rules.yaml
...
**Files:**
- Create: `skills/paarth-learn-loop/SKILL.md`
...
Expected: `FAIL: paarth-learn-loop SKILL.md missing`.

- [ ] **Step 3a: Create `skills/paarth-learn-loop/SKILL.md`**

```bash
mkdir -p skills/paarth-learn-loop
```
...
- **Learning loop** — `~/.paarth/brain/patterns.jsonl` store with promote/decay/protect/prune. Classifier prepends matched chains when `successRate ≥ 0.6 AND useCount ≥ 5`.
...
- **Budget alerts + auto-downgrade** — `bin/paarth-cost-alerts`, `~/.paarth/cost/budget.json`, `~/.paarth/auto-downgrade.flag`. Tiered alerts at 50/75/90/100% of daily budget.
- **`paarth-learn-loop` skill** — user-facing skill for the learning loop.
- **`cost-budget` skill** — user-facing skill for budget enforcement.
- **`bin/paarth-patterns`** — pattern store CLI.
- **`bin/paarth-cost-alerts`** — alert emitter + flag manager.
- **`~/.paarth/defaults.toml`** — single source of truth for magic numbers.

### Changed
- `auto-fallback` skill honors `auto-downgrade.flag` for in-Anthropic tier shifts (precedence: budget > rate-limit > preference).
- `paarth-classify` reads `patterns.jsonl` after `rules.yaml`.
- `paarth-tracker.sh` writes 4-dim records.
- `paarth-distill.sh` calls `paarth-patterns promote && decay` at session Stop.

### Migration
- v1 `calls.jsonl` records auto-detected and read transparently (no rewrite).
- Backup of pre-Wave-1 `calls.jsonl` saved to `~/.paarth/cost/calls.v1.jsonl.bak`.
- Idempotency marker at `~/.paarth/.wave-1.installed`.
...
```markdown
| ✨ **Self-improving classifier (v2.4)** | `~/.paarth/brain/patterns.jsonl` learns from every Stop. Successful chains promote; stale ones decay. Plus 9-event hooks lifecycle, budget alerts, and auto-downgrade at 90% spend. |
```
...
```bash
git add skills/paarth-learn-loop/SKILL.md skills/cost-budget/SKILL.md \
...
Expected: 7 adapter dirs (`adapters/{aider,codex,continue,copilot,cursor,gemini,windsurf}`) regenerated. New skills (`paarth-learn-loop`, `cost-budget`) propagate as rule files. Hooks remain Claude Code-only.
...
1. **Spec coverage.** Every Wave 1 component in `docs/superpowers/specs/2026-05-08-paarth-v3-upgrade-design.md` §6 has at least one task here:
