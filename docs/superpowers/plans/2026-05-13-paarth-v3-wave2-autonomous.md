# PAARTH v3 — Wave 2 (Autonomous & Safe, v2.5.0) Implementation Plan
...
**Architecture:** Four independent threads writing into `~/.paarth/` subdirs:
...
Each thread is independently testable. **Wave 1 is a prerequisite** (patterns.jsonl powers autopilot prediction; cost budget gates autopilot pause; UserPromptSubmit hook hosts the AIDefence call). Spec source: `docs/superpowers/specs/2026-05-08-paarth-v3-upgrade-design.md` §7 + §9.
...
**AIDefence (§7.1):**
- `bin/paarth-aidefence` — scan/enable/disable/list/feedback CLI
...
**Observability (§7.3):**
- `bin/paarth-trace` — read spans.jsonl, build tree, ASCII print with bottleneck flag
- `bin/paarth-metrics` — aggregate counter/gauge/histogram over period
- `bin/paarth-obs-rotate` — daily rotation + 30d retention
...
**Autopilot (§7.4):**
- `bin/paarth-autopilot` — enable/disable/config/status/predict/iter
...
- `hooks/paarth-prompt-submit.py` — call `paarth-aidefence scan`, honor critical/high decisions
- `hooks/paarth-tracker.sh` — emit span start/end on Bash + Edit + Write events; emit metrics
- `hooks/paarth-distill.sh` — apply EMA decay to AIDefence effectiveness; rotate obs daily
- `hooks/paarth-state-init.sh` — scaffold `aidefence/`, `obs/`, `autopilot/` subdirs
- `skills/paarth/brain/rules.yaml` — five specialist routing rules; +5 obs/autopilot rules
...
- `bin/paarth-classify` — recognize specialist routing; prefer specialist when `chain_len > 4 OR complexity == complex`

### Runtime state created at install

- `~/.paarth/aidefence/patterns.json` — copied from skill, mutable for adaptive learning
- `~/.paarth/aidefence/learned.jsonl` — EMA feedback log
- (NO `~/.paarth/aidefence/enabled` flag — default off, opt-in via `paarth-aidefence enable`)
- `~/.paarth/obs/spans.jsonl` — touched
- `~/.paarth/obs/metrics.jsonl` — touched
- `~/.paarth/autopilot/state.json` — initialized to `{enabled:false}`
- `~/.paarth/.wave-2.installed` — idempotency marker
...
Expected: `bash: bin/paarth-aidefence: No such file`.
...
- [ ] **Step 4: Create `bin/paarth-aidefence`**

```bash
#!/usr/bin/env bash
# paarth-aidefence — per-prompt injection + PII scanner.
# Subcommands: scan <text> | enable | disable | status | list | feedback <pattern-id> <accurate|inaccurate>
set -euo pipefail

ROOT="$HOME/.paarth/aidefence"
...
usage() {
  cat <<EOF
Usage: paarth-aidefence <subcommand> [args]
...
case "$cmd" in
  ...
  feedback)
    ...
    if [[ -z "$pid" || -z "$verdict" ]]; then
      echo "Usage: paarth-aidefence feedback <pattern-id> <accurate|inaccurate>" >&2
      exit 1
    fi
    case "$verdict" in
      accurate)   was=true ;;
...
```bash
git add bin/paarth-aidefence skills/aidefence/patterns.json test/test-aidefence-scan.sh
...
**Files:**
- Modify: `bin/paarth-aidefence` (no change — already handled in Task 1)
...
**Files:**
- Modify: `hooks/paarth-prompt-submit.py` (call scanner when enabled flag present)
...
- [ ] **Step 3: Modify `hooks/paarth-prompt-submit.py`**
...
aidefence_enabled = os.path.exists(
    os.path.expanduser("~/.paarth/aidefence/enabled")
)
if aidefence_enabled:
    aidefence_bin = shutil.which("paarth-aidefence") or os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "bin", "paarth-aidefence"
    )
    try:
        r = subprocess.run(
...
```bash
git add hooks/paarth-prompt-submit.py test/test-aidefence-hook.sh
...
**Files:**
- Modify: `bin/paarth-aidefence` (no change — Task 1 ships feedback subcommand)
...
- [ ] **Step 2: Create `commands/aidefence.md`** — slash dispatcher that forwards args to `bin/paarth-aidefence`.
...
<List of trigger phrases — must match classifier rules in skills/paarth/brain/rules.yaml.>
...
<When to escalate back to paarth-brain or to a different specialist.>
```
...
**Files:**
- Modify: `skills/paarth/brain/rules.yaml` (+5 specialist rules)
- Modify: `bin/paarth-classify` (prefer specialist when `chain_len > 4 OR complexity == complex`)
...
- [ ] **Step 3: Classifier preference (in `bin/paarth-classify`)**
...
```bash
git add skills/paarth/brain/rules.yaml bin/paarth-classify test/test-agents-routing.sh
...
**Files:**
- Create: `bin/paarth-obs` — small helper bin used by hooks to append span/metric records
...
LAST_S=$(tail -n1 "$TMPHOME/.paarth/obs/spans.jsonl")
echo "$LAST_S" | jq -e '.op == "test-op" and .traceId == "t-aaaa"' >/dev/null \
  || { echo "FAIL: span record: $LAST_S"; exit 1; }

LAST_M=$(tail -n1 "$TMPHOME/.paarth/obs/metrics.jsonl")
...
- [ ] **Step 2: Implement `bin/paarth-obs`**
...
```bash
git add bin/paarth-obs test/test-obs-emit.sh
git commit -m "feat(obs): bin/paarth-obs append spans + metrics (canonical JSONL shape)"
```
...
**Files:**
- Create: `bin/paarth-trace`
- Test: `test/test-obs-trace.sh`

- [ ] **Step 1: Write the failing test**

Seed `spans.jsonl` with a 3-span tree (root + 2 children), invoke `bin/paarth-trace <traceId>`, assert ASCII output:
...
- [ ] **Step 2: Implement `bin/paarth-trace`**

Python heredoc that reads `~/.paarth/obs/spans.jsonl`, filters by `traceId`, builds parent-child via `parentSpanId`, prints a sorted tree with `├─` / `└─` connectors. Bottleneck = span whose `(endMs - startMs)` exceeds the p95 across the same `op` in the file.

- [ ] **Step 3: Test + commit**

```bash
git add bin/paarth-trace test/test-obs-trace.sh
...
**Files:**
- Create: `bin/paarth-metrics`
- Test: `test/test-obs-metrics.sh`

- [ ] **Step 1: Write the failing test**

Seed `metrics.jsonl` with mixed counter/gauge/histogram records over `today` and `week` ranges. `paarth-metrics today` must:
...
- [ ] **Step 2: Implement `bin/paarth-metrics`**

Python heredoc following the same range parsing as `bin/paarth-cost` (`today | week | all`). Output: table format by default, JSON via `--json`. Anomaly detection (rolling mean + 2σ over last 100 per name) writes a flag line below the table but never halts.

- [ ] **Step 3: Test + commit**

```bash
git add bin/paarth-metrics test/test-obs-metrics.sh
...
**Files:**
- Modify: `hooks/paarth-tracker.sh` (emit `span`-start at PreToolUse equivalent — folded into PostToolUse for simplicity; emit `agent_token_usage` metric)
- Test: `test/test-obs-tracker.sh`

- [ ] **Step 1: Write the failing test**

Pipe a PostToolUse payload with `tool_response.usage` into `paarth-tracker.sh`. Assert `~/.paarth/obs/metrics.jsonl` gained an `agent_token_usage` record and `~/.paarth/obs/spans.jsonl` gained one span entry with `op == "tool.<Bash|Edit|Write|...>"`.

- [ ] **Step 2: Append span + metric emission to `paarth-tracker.sh`**
...
"$SCRIPT_DIR/../bin/paarth-obs" span \
...
"$SCRIPT_DIR/../bin/paarth-obs" metric \
...
```bash
git add hooks/paarth-tracker.sh test/test-obs-tracker.sh
...
**Files:**
- Create: `bin/paarth-obs-rotate`
- Modify: `hooks/paarth-distill.sh` (call rotate at Stop, once per day via marker)
- Test: `test/test-obs-rotation.sh`

- [ ] **Step 1: Implement `bin/paarth-obs-rotate`**

Bash + python heredoc:
1. If `spans.jsonl` is non-empty AND `~/.paarth/obs/.last-rotate-YYYYMMDD` (today) is absent:
...
Add at top of `paarth-distill.sh` (alongside the pattern promote+decay block):
...
```bash
git add bin/paarth-obs-rotate hooks/paarth-distill.sh test/test-obs-rotation.sh
...
Skill explains `paarth-trace <traceId>` and `paarth-metrics [today|week|all]` workflows. Slash command forwards args to the right bin based on subcommand.
...
TMPHOME=$(mktemp -d)
trap 'rm -rf "$TMPHOME"' EXIT
mkdir -p "$TMPHOME/.paarth/autopilot"
...
- [ ] **Step 2: Implement `bin/paarth-autopilot`**
...
2. `~/.paarth/brain/routes.jsonl` records with `outcome:halt`
3. `tasks.md` in cwd if present

- [ ] **Step 3: Test + commit**

```bash
git add bin/paarth-autopilot test/test-autopilot-state.sh
...
**Files:**
- Modify: `bin/paarth-autopilot` (add `predict` subcommand)
- Test: `test/test-autopilot-predict.sh`

- [ ] **Step 1: Write the failing test**

Seed `~/.paarth/brain/patterns.jsonl` with one high-confidence pattern matching a known task signal. Seed pending tasks list. `paarth-autopilot predict` must:
...
Python heredoc that:
1. Reads `~/.paarth/brain/patterns.jsonl`.
...
```bash
git add bin/paarth-autopilot test/test-autopilot-predict.sh
...
**Files:**
- Modify: `bin/paarth-autopilot` (add `iter` subcommand; budget gate)
- Test: `test/test-autopilot-budget-gate.sh`

- [ ] **Step 1: Write the failing test**

Set up budget at $20/day, calls.jsonl at 96% spend. `paarth-autopilot iter` must:
...
```python
# 1. Check ~/.paarth/auto-downgrade.flag → if present AND spend >= 0.9 budget → pause.
#    Otherwise:
# 2. Run predict.
...
#    delaySeconds = min(PAARTH_CACHE_TTL_S or 270, 300)
```
...
```bash
git add bin/paarth-autopilot test/test-autopilot-budget-gate.sh
...
- Modify: `skills/paarth/brain/rules.yaml` (+autopilot rule)
...
```bash
git add skills/autopilot/SKILL.md commands/autopilot.md skills/paarth/brain/rules.yaml test/test-autopilot-docs.sh
...
- Modify: `hooks/paarth-state-init.sh` (add `aidefence/`, `obs/`, `autopilot/` subdirs)
...
[[ -f "$HOME/.paarth/autopilot/state.json" ]] || echo '{"enabled":false,"iterations":0,"history":[]}' > "$HOME/.paarth/autopilot/state.json"
...
```bash
git add bench/prompts.jsonl install.sh hooks/paarth-state-init.sh test/test-install-wave2.sh
...
for p in codex gemini cursor windsurf copilot continue aider; do
  ./bin/paarth-compile --platform "$p" --output adapters/"$p"/templates/...   # platform-specific path
done
```
...
1. **Spec coverage.** Every Wave 2 component in `docs/superpowers/specs/2026-05-08-paarth-v3-upgrade-design.md` §7 has at least one task:
...
7. **Budget gate precedence.** Autopilot iter checks `~/.paarth/auto-downgrade.flag` BEFORE running predict, ensuring `budget > rate-limit > preference` order from spec §9.
