# PAARTH v3 — Wave 3 (Methodology & Quality, v2.6.0 / v3.0.0) Implementation Plan
...
**Architecture:** Three independent threads writing into `~/.paarth/` subdirs:
...
**SPARC (§8.1):**
- `bin/paarth-sparc` — init/gate/advance/report/status CLI
...
**Testgen (§8.2):**
- `bin/paarth-testgen` — scan/suggest/status CLI
...
**Diff-risk (§8.3):**
- `bin/paarth-diff-risk` — classify/impact/risk/reviewers/report CLI
...
- `bin/paarth-classify` — new rules for `sparc | spec | PRD | methodology | gate` (Wave 3 §8.1 routing) and `coverage | untested | testgen` (§8.2 routing). diff-risk routing via existing `review`/`ship` skills.
- `skills/paarth/brain/rules.yaml` — +3 rules (`sparc`, `testgen`, `diff-risk`).
- `skills/review/SKILL.md` — adds a "Diff-risk pre-check" section that calls `paarth-diff-risk` and folds findings into the 6-point checklist.
...
- `hooks/paarth-state-init.sh` — add the same three subdirs (idempotent).

### Runtime state created at install

- `~/.paarth/sparc/` (empty until `sparc init` creates `<slug>/`)
- `~/.paarth/testgen/min-coverage.txt` (default `70`)
- `~/.paarth/testgen/cov-cmd.txt` (empty unless user overrides)
- `~/.paarth/diff/` (empty until first risk report)
- `~/.paarth/.wave-3.installed`
...
`bin/paarth-sparc` (Bash + python heredoc). Subcommand `init <slug>` creates `~/.paarth/sparc/<slug>/` with `state.json`:
...
```bash
git add bin/paarth-sparc test/test-sparc-init.sh
...
| 4 (refinement) | every AC has a matching test name (heuristic: AC label tokens appear in `it(`/`test(`/`describe(`); coverage ≥ threshold from `~/.paarth/testgen/min-coverage.txt`; review status `LGTM` |
...
- Modify: `skills/paarth/brain/rules.yaml` (+sparc rule)
...
**Files:**
- Create: `bin/paarth-testgen`
- Test: `test/test-testgen-scan.sh` + 2 fixtures

`scan` subcommand:

1. Read `~/.paarth/testgen/cov-cmd.txt` if present (user override).
...
4. Cache the normalized report at `~/.paarth/testgen/last-report.json`.
...
`status` prints current coverage, threshold, gap count, top file. `--json` mode for `ship`/`review` to consult. Threshold lives in `~/.paarth/testgen/min-coverage.txt`; default 70 when missing.
...
- Modify: `skills/paarth/brain/rules.yaml` (+testgen rule)
...
Compose classifier + impact + risk factors + reviewers into the markdown report from spec §8.3. Cache at `~/.paarth/diff/last.json` for `ship`/`review` to consult.
...
- Modify: `skills/paarth/brain/rules.yaml` (+diff-risk rule)
...
Add a section to the review procedure: "Step 0 — run `paarth-diff-risk` for pre-check; fold impact level into the verdict." Test asserts `skills/review/SKILL.md` mentions `paarth-diff-risk` and `critical|high`.
...
Both skills consult `~/.paarth/testgen/last-report.json` if present. `review` mentions coverage in its output; `ship` warns when coverage dropped vs the cached previous report.
...
- Modify: `hooks/paarth-state-init.sh`
...
install.sh scaffolds `~/.paarth/{sparc,testgen,diff}/`, seeds `min-coverage.txt=70`, drops `.wave-3.installed` marker. Idempotent.
...
1. **Spec coverage.** Every Wave 3 component in `docs/superpowers/specs/2026-05-08-paarth-v3-upgrade-design.md` §8 has at least one task:
   - §8.1 SPARC → Tasks 1–6. ✓
