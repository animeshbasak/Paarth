...
def test_bench_restores_environment():
    before_flag = os.environ.get("SUPERAGENT_MEMORY_VECTOR")
    before_backend = os.environ.get("SUPERAGENT_MEMORY_VECTOR_BACKEND")
    bench.run_bench(k=3)
    assert os.environ.get("SUPERAGENT_MEMORY_VECTOR") == before_flag
    assert os.environ.get("SUPERAGENT_MEMORY_VECTOR_BACKEND") == before_backend
