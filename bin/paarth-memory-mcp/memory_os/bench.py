...
Run: ``paarth-memory bench [--k 5] [--real]``.
"""
...
def run_bench(*, k: int = 5, embed_fn: Callable[[str], Vector] | None = None) -> dict:
    ...
    saved = {key: os.environ.get(key) for key in ("PAARTH_MEMORY_VECTOR", "PAARTH_MEMORY_VECTOR_BACKEND")}
    os.environ["PAARTH_MEMORY_VECTOR"] = "on"
    os.environ["PAARTH_MEMORY_VECTOR_BACKEND"] = "memory"
    vector.reset_store()
    try:
        ids: list[str] = []
        for content, _, _ in CORPUS:
