...
``PAARTH_MEMORY_VECTOR`` is on, so the default install pays nothing.

Store selection (``PAARTH_MEMORY_VECTOR_BACKEND``):
...
def _build_store() -> VectorStore:
    backend = os.environ.get("PAARTH_MEMORY_VECTOR_BACKEND", "auto").strip().lower()
    if backend == "memory":
        return InMemoryStore()
    if backend in ("qdrant", "auto"):
        try:
            url = os.environ.get("PAARTH_MEMORY_QDRANT_URL", "http://127.0.0.1:6333")
