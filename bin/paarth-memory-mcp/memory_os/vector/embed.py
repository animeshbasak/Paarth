...
This mirrors PAARTH's "local first" ethos — no mandatory cloud call. The
...
def _ollama_embed(text: str, *, timeout: float) -> Vector:
    httpx = _import_httpx()
    url = os.environ.get("PAARTH_MEMORY_OLLAMA_URL", DEFAULT_OLLAMA_URL).rstrip("/")
    model = os.environ.get("PAARTH_MEMORY_EMBED_MODEL", DEFAULT_OLLAMA_MODEL)
    try:
        resp = httpx.post(
...
def _openrouter_embed(text: str, *, timeout: float) -> Vector:
    ...
    url = os.environ.get("PAARTH_MEMORY_OPENROUTER_URL", DEFAULT_OPENROUTER_URL).rstrip("/")
    model = os.environ.get("PAARTH_MEMORY_CLOUD_EMBED_MODEL", DEFAULT_OPENROUTER_MODEL)
    try:
        resp = httpx.post(
            f"{url}/embeddings",
