...
Single file at ~/.paarth/memory-os/memory.db. Tables:
...
DEFAULT_DB_DIR = Path(os.environ.get("PAARTH_MEMORY_HOME", "~/.paarth/memory-os")).expanduser()
...
def pin_entry(conn: sqlite3.Connection, entry_id: str, pin_dir: Path) -> Path:
    ...
    pin_path.write_text(
        f"<!-- pinned from paarth-memory entry {entry_id} ({row['kind']}) -->\n{row['content']}\n",
        encoding="utf-8",
    )
...
def telemetry_enabled() -> bool:
    """Local-only counters; on by default, opt out with PAARTH_MEMORY_TELEMETRY=off."""
    return os.environ.get("PAARTH_MEMORY_TELEMETRY", "on").strip().lower() not in (
        "off", "0", "false", "no",
    )
...
def bump_counter(conn: sqlite3.Connection, name: str, namespace: str = "", by: int = 1) -> None:
    ...
    Counters never leave the SQLite file — they exist so `paarth-memory
...
def stats(conn: sqlite3.Connection) -> dict:
    """Aggregate store stats + usage counters for `paarth-memory stats`."""
