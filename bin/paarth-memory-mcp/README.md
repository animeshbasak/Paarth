...
MCP server providing persistent multi-tier memory for PAARTH across Claude Code, Cursor, Antigravity, Gemini CLI, and GitHub Copilot.
...
A second console script, `paarth-memory`, runs maintenance jobs (used by
hooks or cron — not the MCP transport):

```bash
paarth-memory decay --dry-run          # report stale entries, no changes
paarth-memory decay                     # archive entries older than 90d AND idle 30d
paarth-memory decay --max-age-days 180 --idle-days 60 --namespace <ns>
paarth-memory dedup --dry-run           # report near-duplicate merges (needs PAARTH_MEMORY_VECTOR=on)
paarth-memory dedup                      # merge entries with cosine ≥0.92, namespace-scoped
paarth-memory cron install              # schedule weekly decay (launchd on macOS, crontab elsewhere)
paarth-memory cron status
paarth-memory cron uninstall
```
...
- `~/.paarth/memory-os/memory.db` — SQLite + FTS5
- `~/.paarth/memory-os/pinned/` — promoted entries (markdown)
- Override the root via `PAARTH_MEMORY_HOME`.
...
- **Embeddings:** local-first — Ollama (`nomic-embed-text`, 768-dim) by
  ...
  is down.
- **Store backend** (`PAARTH_MEMORY_VECTOR_BACKEND`): `auto` (default —
...
Relevant env: `PAARTH_MEMORY_QDRANT_URL`, `PAARTH_MEMORY_OLLAMA_URL`,
`PAARTH_MEMORY_EMBED_MODEL`.
...
- **Remote Qdrant** — pointing `PAARTH_MEMORY_QDRANT_URL` off-localhost
