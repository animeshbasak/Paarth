#!/usr/bin/env bash
# paarth-limit-watch.sh — UserPromptSubmit hook
#
# Reads the Claude Code prompt-submit JSON from stdin, polls `paarth-cost today --json`,
# and writes a warning marker to ~/.paarth/limit-warn.json when:
#   - spend > 80% of plan limit
#   - time to 5h reset < 30min
#   - 429 burst >= 3 in 60s
#
# When auto-fallback flag is "on" AND no in-flight tool calls, it invokes
# `paarth-switch auto-suggest` to surface a switch prompt. Idempotent + flock-safe.
#
# Hook type: UserPromptSubmit (NOT PreToolUse — see Engineer C audit).

set -euo pipefail

STATE_DIR="$HOME/.paarth"
...
COST_BIN=""
for c in \
  "$SCRIPT_DIR/../bin/paarth-cost" \
  "$HOME/.local/bin/paarth-cost" \
  "$(command -v paarth-cost 2>/dev/null || true)"; do
  if [[ -n "$c" && -x "$c" ]]; then
    COST_BIN="$c"
    break
  fi
...
if [[ -z "$COST_BIN" ]]; then
  log "paarth-cost not found — skipping"
  exit 0
fi
...
if [[ "$AUTO" == "on" ]]; then
  ...
  if [[ "${IN_FLIGHT:-0}" -eq 0 ]]; then
    SWITCH_BIN="$SCRIPT_DIR/../bin/paarth-switch"
