#!/usr/bin/env python3
# paarth-safety.py — PreToolUse reversibility-aware action gate
#
...
#   1. ~/.paarth/safety/allow.txt        — one regex per line
#   2. CLAUDE.md "## PAARTH Safety Allow" section bullets
#   3. settings.local.json permissions.allow (Claude already honors)
#
# Bypass: PAARTH_SAFETY=off in env disables the gate entirely.
#
# Local-only mode (~/.paarth/local-only present): network egress patterns
...
# allowed_model_tiers (see bin/paarth-org-policy).
MODEL_SELECTORS = [
    re.compile(r"--model[=\s]+([\w./:-]+)", re.I),
    re.compile(r"\b(?:ANTHROPIC|PAARTH|CLAUDE)_MODEL=([\w./:-]+)", re.I),
    re.compile(r"\bpaarth-switch\s+to\s+([\w./:-]+)", re.I),
]
...
def load_allow_patterns() -> list[re.Pattern]:
    ...
    for src in sources:
        ...
        if src.name == "CLAUDE.md":
            # Extract bullets under "## PAARTH Safety Allow" through next heading
            m = re.search(
                r"^##+\s*PAARTH\s+Safety\s+Allow\s*$(.*?)(?=^##+\s|\Z)",
...
def load_org_policy() -> dict:
    """Read ~/.paarth/org-policy.json (mirrors bin/paarth-org-policy)."""
    try:
        with open(Path.home() / ".paarth" / "org-policy.json") as f:
            data = json.load(f)
...
def classify_model_policy(command: str) -> str | None:
    ...
    if os.environ.get("PAARTH_ORG_POLICY") == "off":
        return None
...
def classify_bash(command: str) -> tuple[str | None, str | None]:
    ...
    if (Path.home() / ".paarth" / "local-only").exists():
...
def emit(decision: str, reason: str = "", *, exit_code: int = 0) -> None:
    ...
    if decision == "deny":
        sys.stderr.write(f"[paarth-safety] BLOCKED: {reason}\n")
        sys.exit(2)
    out = {
        "hookSpecificOutput": {
...
def main() -> None:
    if os.environ.get("PAARTH_SAFETY") == "off":
        emit("allow", "safety gate disabled via PAARTH_SAFETY=off")
        return
    ...
    if label == "fork bomb" or label.startswith("dd to raw disk") or "mkfs" in label:
        emit("deny", f"paarth-safety refuses: {label}")
        return

    emit("ask", f"paarth-safety: {label} — confirm before proceeding")
