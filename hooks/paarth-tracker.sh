...
# Wave 2: emit span + token-usage metric via bin/paarth-obs (best effort).
emit_obs_records() {
  local payload="$1"
  local tool_name="$2"

  local obs_bin
  obs_bin="$(command -v paarth-obs 2>/dev/null || true)"
  if [[ -z "$obs_bin" ]]; then
    local script_dir
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    if [[ -x "$script_dir/../bin/paarth-obs" ]]; then
      obs_bin="$script_dir/../bin/paarth-obs"
    fi
  fi
  [[ -z "$obs_bin" ]] && return 0
...
# Detect which paarth tool was used (Bash branch only — preserves existing logic)
TOOL_TYPE=""
