...
# One-time v4.0 migration: whole legacy state dir moves to the new name.
LEGACY_ROOT="${HOME}/.superagent"
if [[ -d "$LEGACY_ROOT" && ! -d "$ROOT" ]]; then
  mv "$LEGACY_ROOT" "$ROOT"
  echo "migrated: $LEGACY_ROOT -> $ROOT"
fi
